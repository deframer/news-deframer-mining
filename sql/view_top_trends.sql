...
WITH ranked_global_trends AS (
    SELECT
        ...
        outlier_ratio,
        time_slice,
        -- Rank the same word by its highest Outlier Score within this week
        ROW_NUMBER() OVER (
            PARTITION BY stem
...
SELECT
    ...
    utility,
    outlier_ratio,
    time_slice as peak_date
FROM ranked_global_trends
WHERE rn = 1  -- Keep only the strongest signal for this word
