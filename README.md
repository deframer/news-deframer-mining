...
Models are lazily loaded: on first use they download into ignored `models/spacy/` and `models/memolon/` directories, then stay cached in memory for the running process. In Docker, `MODEL_ROOT=/app/models` is used so the volume mount can be shared.
